...
namespace Opsive.UltimateInventorySystem.Core.InventoryCollections
{
    ...
    /// </summary>
    [Serializable]
    public class MyItemTransactionCollection : ItemCollection
    {
        [Tooltip(
        ...
        [Tooltip(
            "The hotbar collection where the item will be transferred to, if there is another copy of this item is there already")]
        [SerializeField] protected string m_HotbarItemSlotCollectionName;
        [Tooltip(
        ...
        {
            if (m_HotbarItemSlotCollectionName != "" && m_Inventory.GetItemCollection(m_HotbarItemSlotCollectionName).HasItem(itemInfo))
            {
                m_AddingToCollection = m_Inventory.GetItemCollection(m_HotbarItemSlotCollectionName);
                return itemInfo;
            }
