...
namespace Opsive.UltimateInventorySystem.Core
{
    ...
    public class InventorySystemManager : MonoBehaviour, IInventorySystemManager
    {
        ...
        /// <summary>
        /// A function that prints in the console a readable list of all the names and IDs of the objects that are currently registered in the manager.
        /// </summary>
        public void PrintGetReadableRegisteredObjectsNamesAndIDs()
        {
            Debug.Log(GetReadableRegisteredObjectsNamesAndIDs());
        }

        /// <summary>
        /// A function that returns a readable list of all the names and IDs of the objects that are currently registered in the manager.
        /// </summary>
        /// <returns>A string containing all the names and Ids of the objects registered in the manager.</returns>
        public string GetReadableRegisteredObjectsNamesAndIDs()
        {
            var text = "";

            text += "Item Categories:\n";
            text += GetReadableRegisteredObjectsNameAndIDs(m_Register.ItemCategoryRegister);
            ...
            foreach (var definition in allDefinitions) {
                index++;
                text += $"\t[{index}] {definition} ({definition.ID} | category: {definition.Category} ({definition.Category.ID}))\n";
            }

            text += "\n";
            text += "Items:\n";
            var allItems = m_Register.ItemRegister.GetAll();
            index = 0;
            foreach (var item in allItems) {
                index++;
                // Item already prints with the ID by default.
                text += $"\t[{index}] {item} | definition: {item.ItemDefinition} ({item.ItemDefinition.ID}))\n";
            }
            
            text += "\n";
            text += "Crafting Categories:\n";
            text += GetReadableRegisteredObjectsNameAndIDs(m_Register.CraftingCategoryRegister);
            
            text += "\n";
            text += "Recipes:\n";
            text += GetReadableRegisteredObjectsNameAndIDs(m_Register.CraftingRecipeRegister);
            
            text += "\n";
            text += "Currencies:\n";
            text += GetReadableRegisteredObjectsNameAndIDs(m_Register.CurrencyRegister);
        ...
        /// A function that returns a readable list of all the names and IDs of the objects that are currently registered in the manager.
        /// </summary>
        /// <param name="register">The register with the name and IDs to turns into text.</param>
        /// <typeparam name="T">The registered element type.</typeparam>
        /// <returns>A string containing all the names and Ids of the objects registered in the manager.</returns>
        protected string GetReadableRegisteredObjectsNameAndIDs<T>(InventoryObjectRegister<T> register) where T : class, IObjectWithIDReadOnly
        {
            var text = "";
            var allElements = register.GetAll();
            var index = 0;
            foreach (var element in allElements) {
                index++;
                text += $"\t[{index}] {element} ({element.ID})\n";
            }
