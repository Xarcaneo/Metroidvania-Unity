...
namespace Opsive.UltimateInventorySystem.UI.Grid
{
    ...
    public class LayoutGroupNavigation : MonoBehaviour
    {
        ...
        protected VerticalLayoutGroup m_VerticalLayout;
        protected HorizontalLayoutGroup m_HorizontalLayout;
        ...
        protected virtual bool TryGetSelectableChild(int index, out Selectable selectable)
        {
        ...
        protected virtual void SetOneDimensionalNavigation(bool horizontal)
        {
            Selectable previous = null;
            Selectable firstElement = null;
            for (int i = 0; i < transform.childCount; i++) {
                if (TryGetSelectableChild(i, out var selectable)) {

                    //Get the first element.
                    if (m_GridWrap && firstElement == null) {
                        firstElement = selectable;
                    }
                    
                    if (horizontal) {
                        previous = HorizontalNavigation(selectable, previous, true);
                    } else {
                        previous = VerticalNavigation(selectable, previous, true);
                    }
                }
            }

            //Grid wrap.
            if (m_GridWrap && firstElement != null && firstElement != previous) {
                
                if (horizontal) {
                    HorizontalNavigation(firstElement, previous, false);
                } else {
                    VerticalNavigation(firstElement, previous, false);
                }
            }
        }
        ...
        /// <param name="setOthersToNull">Set the other navigation direction to null.</param>
        ...
        public virtual void SetGridLayoutGroupNavigation(GridLayoutGroup gridLayoutGroup)
        {
        ...
        /// </summary>
        protected virtual void SetChildrenAutoNavigation()
        {
