...
namespace Opsive.UltimateInventorySystem.UI.Grid
{
    ...
    {
        [Serializable]
        public enum DisableElementOptions
        {
            DoNotDisableElements,
            DisableAfterElementCount,
            DisableAfterCurrentRow,
            DisableAfterCurrentColumn,
            CustomDisableIndex,
            CustomDisableIndexPlusElementCount
        }
        ...
        [SerializeField] protected DisableElementOptions m_DisableElementOption = DisableElementOptions.DoNotDisableElements;
        ...
        public bool SetRealElementCountAsMax {
            get { return m_SetRealElementCountAsMax; }
            set { m_SetRealElementCountAsMax = value; }
        }
        
        public int MaxElementCount {
            get { return m_MaxElementCount; }
            set { m_MaxElementCount = value; }
        }
        
        public DisableElementOptions DisableElementOption {
            get { return m_DisableElementOption; }
            set { m_DisableElementOption = value; }
        }
        
        public int CustomDisableElementIndex {
            get { return m_CustomDisableElementIndex; }
        ...
        /// <summary>
        /// The Index at which the elements in the Grid should be disabled.
        /// If the amount if negative (-1) that means elements should never be disabled.
        /// </summary>
        /// <returns>The disable index (can be -1)</returns>
        public virtual int GetDisableIndex()
        {
            switch (m_DisableElementOption) {
                case DisableElementOptions.DoNotDisableElements:
                    return -1;
                case DisableElementOptions.DisableAfterElementCount:
                    return ElementCount;
                case DisableElementOptions.DisableAfterCurrentRow:
                    var additionalToCompleteRow = m_GridSize.x - (ElementCount % m_GridSize.x);
                    return ElementCount + additionalToCompleteRow;
                case DisableElementOptions.DisableAfterCurrentColumn:
                    var additionalToCompleteColumn = m_GridSize.y - (ElementCount % m_GridSize.y);
                    return ElementCount + additionalToCompleteColumn;
                case DisableElementOptions.CustomDisableIndex:
                    return CustomDisableElementIndex;
                case DisableElementOptions.CustomDisableIndexPlusElementCount:
                    return ElementCount + CustomDisableElementIndex;
                default:
                    return -1;
            }
        }
        ...
        internal virtual void OnValidate()
        {
            ...
            var content = m_GridEventSystem.Content ?? m_GridEventSystem.transform;

            if (content != null && content.childCount != m_GridSize.x * m_GridSize.y) {
                if (m_GridEventSystem.ButtonPrefab != null) {
                    m_GridEventSystem.SpawnButtons(m_GridSize, false);
                }
            }
        ...
        public void SetParentPanel(DisplayPanel parentPanel)
        {
            ...
            if (m_ParentPanel != null) {
                m_PlayerInput = m_ParentPanel.Manager.PanelOwner?.gameObject?.GetCachedComponent<IPlayerInput>();
            }
        }
